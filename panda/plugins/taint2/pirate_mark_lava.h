...
static const int LAVA_EXPLOITABILITY = 14; // reserved, unused
static const int LAVA_SINGLE_LABEL = 15;
static const int LAVA_POS_LABEL = 16;
...
static inline
void vm_lava_label_buffer(const void *buf, unsigned long len, 
                          lavaint src_filename, lavaint src_ast_node_name,
                          unsigned long linenum, lavaint ins,
                          unsigned label_pos, lavaint label) {
  volatile PandaHypercallStruct phs = {0};
  phs.magic = 0xabcd;
  if (label_pos){
    phs.action = LAVA_POS_LABEL;
  }
  else {
      phs.action = LAVA_SINGLE_LABEL;
  }
  phs.buf = (lavaint) buf;
  phs.len = len;
  if (!label_pos){
