                                        ...
                                        TranslationBlock **last_tb)
{
    ...
    rr_interrupt_request(&cpu->interrupt_request);
#endif
...
static inline bool rr_maybe_finished(void) {
...
int cpu_exec(CPUState *cpu)
{
    ...
    while (!cpu_handle_exception(cpu, &ret)) {
        ...
        while (!cpu_handle_interrupt(cpu, &last_tb) && !rr_maybe_finished()) {
            ...
            if (panda_invalidate_tb
                ...
                // retranslate so block boundary matches for interrupt delivery
                tb_lock();
                tb_phys_invalidate(tb, -1);
                tb_unlock();
                continue;
